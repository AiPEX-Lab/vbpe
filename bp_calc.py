...
def vessel_radius(height_person_cm, age, bmi):
    return -0.258 + 0.029*height_person_cm + 0.006*age + 0.036*bmi

def vessel_thickness(height_person_cm, age, bmi):
    return 0.25 + 0.005*age + 0.005*bmi
    
def bp_calculator(lag, frame_rate, image_path, height_cm, age, bmi):
    ...
    alpha = 0.017
    # r = 0.005 # Original
    r = 0.001*0.5*vessel_radius(height_cm, age, bmi)
    # h = 0.001 # Original
    h = 0.001*vessel_thickness(height_cm, age, bmi)
    # E_0 = 263000 # Original
    E_0 = 1005
    bp = -0.205*((-2/alpha)*np.log(lagf) + (1/alpha)*np.log((2*r*rho*(vessel_length**2))/(h*E_0)))
    return bp

def main_bp(ppg_path_face, ppg_path_hand, image_path, demographic_path):
    ...
    for csv_face, csv_hand, image, h, a, b in zip(csv_face_names, csv_hand_names, image_names, height_cm, age, bmi):
        ...
        d1butter = butter_bandpass_filter_zi(d1, lowcut, highcut, sRate, order=2)
        d2butter = butter_bandpass_filter_zi(d2, lowcut, highcut, sRate, order=2)
        time_delay = peak_detector(d1butter, d2butter)
        bp_predicted = bp_calculator(time_delay, fps, image_name_path, h, a, b)
        bp_values.append(bp_predicted)

        del d1, d2, d1butter, d2butter
        gc.collect()
    # print(csv_face_names, csv_hand_names)
    print('Systolic Blood pressure:', bp_values)
    np.savetxt('SBP_new.csv', bp_values, header='SBP')
